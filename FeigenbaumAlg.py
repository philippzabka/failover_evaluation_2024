...
# Feigenbaum guarantees routing with one link failure
...
def FeigenbaumAlg(s, d, fails, dag):
    ...
    if not already_initialized:
        ...
        # nx.write_qpickle depricated in Networkx 3.x
        with open('routing_rules.gpickle_' + str(len(fails)), 'wb') as f:
        # with open('routing_rules.gpickle', 'wb') as f:
            pickle.dump(dag_copy, f) # we serialize the dag_copy graph, which contains the normal and the alternative routing rules

    with open('routing_rules.gpickle_' + str(len(fails)), 'rb') as f:
    # with open('routing_rules.gpickle', 'rb') as f:
        dag_copy = pickle.load(f)
